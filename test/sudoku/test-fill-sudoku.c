...
bool problem_equal(unsigned int* first, unsigned int* second, unsigned int size) {
...
int main(void) {
    ...
    // create tests
    add_test(tests, create_test("three_by_three", (unsigned int[]){ 2, 0, 0, 0, 0, 0, 0, 1, 0}, 3));
    add_test(tests, create_test("null", (unsigned int[]){ 0, 0, 0, 0}, 2));
    add_test(tests, create_test("irregular_size_1", (unsigned int[]){ 1, 2, 1, 2, 3}, 2));
    add_test(tests, create_test("irregular_size_2", (unsigned int[]){ 1, 2, 3, 4, 0, 5}, 2));
    add_test(tests, create_test("full", (unsigned int[]){ 1, 2, 3, 4, 5, 6, 7, 8, 9}, 3));
    ...
    for(unsigned int i = 0; i < tests->size; i++) {
        ...
        if(strstr(test->path, "irregular_size_1") || strstr(test->path, "irregular_size_2")) {
            assert(sudoku == NULL);
        } else if(strstr(test->path, "three_by_three") || strstr(test->path, "null") || strstr(test->path, "full")) {
            assert(problem_equal(sudoku->problem, test->values, test->dim_size*test->dim_size));
        } else {
