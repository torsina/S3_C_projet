...
unsigned int* _evaluate_merge_problem_solution(unsigned int* individual, Sudoku* sudoku) {
  ...
  for(unsigned int i = 0; i < size; i++) {
    if(sudoku->problem[i] != 0) merge[i] = (unsigned int)sudoku->problem[i];
    else merge[i] = individual[i] + 1;
  }
  return merge;
}


unsigned int _evaluate_used_numbers_duplicates(unsigned int* used_values, unsigned int used_size) {
  ...
  for(unsigned int i = 0; i < used_size; i++) {
    ...
    while(j < unique_size && result) {
      if(unique_values[j] == used_values[i]) result = false;
      else unique_values[unique_size] = used_values[i];
      j++;
    }
...
unsigned int _evaluate_check_row(unsigned int* merge, Sudoku* sudoku, unsigned int index) {
  ...
  unsigned int duplicates = _evaluate_used_numbers_duplicates(used_values, used_size);
  free(used_values);
  return duplicates;
}

bool _evaluate_check_column(unsigned int* merge, Sudoku* sudoku, unsigned int index) {
  unsigned int dim_size = sudoku_get_dim_size(sudoku);
...
unsigned int evaluate(unsigned int* individual, const void* sudoku) {
  ...
  free(merge);
  // afraid that score could be negative if too much duplicates(because an error can be counted multiple times)
  // so squaring score to be sure
